...
| STT alle Agents | Azure Speech | Speech-Resource in EU-Region, z. B. `swedencentral`, `westeurope`, `germanywestcentral` |
...
Azure Speech ist die Default-Wahl in diesem Repo, weil STT und TTS über denselben Microsoft-DPA/EU-Region-Stack laufen können. Lege die Speech-Resource in einer EU-Region nahe am Server an, z. B. `swedencentral`, `westeurope`, `germanywestcentral` oder `francecentral`.
